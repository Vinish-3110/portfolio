...
export interface HyperspeedColors {
  ...
  sticks?: number | string | (number | string)[];
}
...
class App {
  ...
  constructor(container: HTMLElement, options: any = {}) {
    ...
    if (typeof (THREE as any).Timer === 'function') {
      ...
      if (this.timer.connect) this.timer.connect(document);
    } else {
  ...
  loadAssets() {
    ...
    return new Promise(resolve => {
      ...
      const manager = new THREE.LoadingManager(safeResolve);
...
interface HyperspeedProps {
  ...
  lightMode?: boolean;
  className?: string;
  onAccelerate?: (isAccelerating: boolean) => void;
}
...
const Hyperspeed: React.FC<HyperspeedProps> = ({
  ...
  useEffect(() => {
    ...
    // Forward speed hooks
    const originalSpeedUp = mergedOptions.onSpeedUp;
    const originalSlowDown = mergedOptions.onSlowDown;
    mergedOptions.onSpeedUp = (ev: any) => {
      if (originalSpeedUp) originalSpeedUp(ev);
      if (onAccelerate) onAccelerate(true);
    };
    mergedOptions.onSlowDown = (ev: any) => {
      if (originalSlowDown) originalSlowDown(ev);
  ...
  }, [effectOptions, lightMode, onAccelerate]);

  return <div id="lights" ref={hyperspeed} className={className} />;
};
